...
async def personal_info_menu(message: types.Message, state: FSMContext):
    ...
    buttons = [
        ...
        [KeyboardButton(text=texts['participate_in_the_lottery_button'][user_language]),
         ...
         KeyboardButton(text=texts['buy_ticket_for_another_user_button'][user_language])],  # Добавили user_language
...
async def participate_lottery(message: types.Message, state: FSMContext):
    ...
    with connect_db() as conn:
        ...
        await state.update_data(lottery_id=lottery_id, ticket_price=ticket_price)
...
async def process_user_info(message: types.Message, state: FSMContext):
    ...
    with connect_db() as conn:
        ...
        await message.reply("Введите номер билета, который хотите купить (1-1000):")
        await state.update_data(lottery_id=lottery_id, ticket_price=ticket_price, target_user_id=target_user_id)

    await state.set_state(LotteryStatesForFriend.awaiting_ticket_number)  # Переводим в состояние ожидания номера билета

@router.message(LotteryStatesForFriend.awaiting_ticket_number)
async def process_ticket_number_for_another_user(message: types.Message, state: FSMContext):
    admin_id = message.from_user.id
    ticket_number = int(message.text)
    ...
    with connect_db() as conn:
        ...
        cursor.execute(
            "UPDATE users SET balance = balance - ? WHERE user_id = ?",
            (ticket_price, admin_id)
        )

        cursor.execute(
            "UPDATE lottery_tickets SET user_id = ?, username = ? WHERE ticket_number = ? AND lottery_id = ?",
            (target_user_id, None, ticket_number, lottery_id)  # Для другого пользователя не указываем имя администратора
        )
        conn.commit()
