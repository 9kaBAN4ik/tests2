...
from database import (set_user_role, get_user_role,get_user_courses, get_user_products,
                      ...
                      get_all_users,add_balance_to_user,connect_db,load_texts,get_user_language,get_current_referral_system,update_referral_rewards,create_referral_system,get_users_with_balance,add_referral_reward,get_referral_reward,get_referral_system_id)# Работа с ролями пользователей
...
async def admin_menu(message: Message):
    ...
    buttons = [
        ...
        [KeyboardButton(text=str(texts['create_referral_system'].get(user_language,"default"))),
         KeyboardButton(text=texts["view_users_with_balance"][user_language])],
...
async def start_lottery(message: Message, state: FSMContext):
    await message.reply("Какая награда будет для 446 утешительных билетов?")
...
async def receive_fund(message: Message, state: FSMContext):
    ...
    await message.reply("Введите процент, который пойдет в реферальную программу (0-100):")
    await state.set_state("waiting_for_referral_percentage")


@router.message(StateFilter("waiting_for_referral_percentage"))
async def receive_referral_percentage(message: Message, state: FSMContext):
    try:
        referral_percentage = float(message.text.strip())
        if not (0 <= referral_percentage <= 100):
            raise ValueError
    except ValueError:
        await message.reply("Ошибка! Введите корректный процент от 0 до 100:")
        return

    await state.update_data(referral_percentage=referral_percentage)
...
async def receive_ticket_price(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    consolation_prize = data.get("consolation_prize")
    fund = data.get("fund")
    referral_percentage = data.get("referral_percentage")

    referral_fund = (fund * referral_percentage) / 100  # Считаем сумму для рефералки
    ...
    with connect_db() as conn:
        ...
        # Создаем новую лотерею с указанным фондом, реферальным процентом и ценой билета
        cursor.execute(
            "INSERT INTO lottery (name, ticket_price, fund, referral_fund, referral_percentage, active) VALUES (?, ?, ?, ?, ?, ?)",
            ("Месячная лотерея", ticket_price, fund, referral_fund, referral_percentage, 1)
        )
        lottery_id = cursor.lastrowid

        # Генерация выигрышных билетов
        ...
        for ticket_number in range(1, 1001):
            ...
            elif consolation_count < 446:  # 446 утешительных билетов
                tickets.append((lottery_id, None, None, ticket_number, 0, consolation_prize))
                consolation_count += 1
            else:  # 500 пустых
    ...
    if not users:
        ...
        user_list = "\n".join([f"ID: {user['user_id']}" for user in users])

        # Отправляем всем пользователям уведомление о начале лотереи
        for user in users:
    ...
    await state.clear()
    await message.reply(f"Лотерея началась! Призовой фонд: {fund}, в рефералку пойдет ({referral_percentage}%), цена билета: {ticket_price}!")
...
async def end_lottery(message: types.Message):
    ...
    with connect_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, fund, referral_percentage FROM lottery WHERE active = 1 ORDER BY id DESC LIMIT 1")
    ...
    lottery_id, total_fund, referral_percentage = active_lottery  # Общий фонд лотереи
    logging.info(f"Общий фонд лотереи: {total_fund} VED")
    logging.info(f"Процент для реферальных выплат: {referral_percentage}%")

    # Получаем ID активной реферальной системы
    ...
    # Получаем выигрышные билеты
    with connect_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id, ticket_number FROM lottery_tickets WHERE prize != 'Пустой'")
    ...
    for ticket in tickets:
        ticket_id, user_id, ticket_number = ticket

        if user_id is None:
            logging.error(f"Ошибка: билет {ticket_id} имеет user_id=None")
            continue
        ...
        if prize_category in winning_categories:
            ...
            logging.info(f"Билет {ticket_id}: {prize_category}, выигрыш = {prize_amount} VED")

            if prize_amount == 0.0:
                logging.warning(f"ВНИМАНИЕ: Билет {ticket_id} получил {prize_amount} VED!")
